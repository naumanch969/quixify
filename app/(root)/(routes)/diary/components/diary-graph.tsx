...
import { getDiaryStats, } from '../utils'
import { useUser } from '@clerk/nextjs'
import { format } from 'date-fns'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { CustomPagination } from '@/components/pagination'

const DiaryGraph = ({ diaries }: { diaries: Diary[] }) => {
    ...
    useEffect(() => {
        if (user?.createdAt) {
            const d = getDiaryStats(diaries, user.createdAt as Date, Number(splitNumber), Number(splitLength), setSplitNumber)
            setData(d)
            ...
            <div className="w-full overflow-x-auto space-x-2 p-1 h-[26rem] overflow-hidden ">
                <div className="w-full h-full flex flex-col gap-4 ">
                    <div className="flex justify-between items-center bg-black rounded-lg p-2 pl-8 ">
                        <h3 className='font-semibold text-3xl ' >Daily Stats</h3>
